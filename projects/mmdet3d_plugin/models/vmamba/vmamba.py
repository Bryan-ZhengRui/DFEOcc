...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            ...
            v3=partial(self.forward_corev2, force_fp32=True, selective_scan_backend="oflex", scan_force_torch=True),
            v4=partial(self.forward_corev2, force_fp32=True, selective_scan_backend="oflex", scan_mode="spiral2d_ccw", scan_force_torch=True),
            v5=partial(self.forward_corev2, force_fp32=True, selective_scan_backend="oflex", scan_mode="spiral2d_cw", scan_force_torch=True),
        )
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        _scan_mode = dict(cross2d=0, unidi=1, bidi=2, spiral2d=3, spiral2d_ccw=4, spiral2d_cw=5, cascade2d=-1).get(scan_mode, None) if isinstance(scan_mode, str) else scan_mode # for debug
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        Bs = cross_scan_fn(Bs.contiguous(), in_channel_first=self.channel_first, out_channel_first=True, one_by_one=False).view(B, 4, -1, L)
        Cs = cross_scan_fn(Cs.contiguous(), in_channel_first=self.channel_first, out_channel_first=True, one_by_one=False).view(B, 4, -1, L)
