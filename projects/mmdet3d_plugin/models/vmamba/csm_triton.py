...
def cross_scan_fwd(x: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0):
    from .scan4bevfeature import rearrange_feature_map
    if in_channel_first:
        B, C, H, W = x.shape
        if scans == 0:
            ...
            y0 = rearrange_feature_map(x, direction='ccw', channel_first=True)
            y0 = y0.view(B, 1, C, H * W).repeat(1, 2, 1, 1)
            y = rearrange_feature_map(x, direction='cw', channel_first=True)
            y = y.view(B, 1, C, H * W).repeat(1, 2, 1, 1)
            y = torch.cat([y0, y], dim=1)
        elif scans == 4:
            y0 = rearrange_feature_map(x, direction='ccw', channel_first=True)
            y = y0.view(B, 1, C, H * W).repeat(1, 4, 1, 1)
        elif scans == 5:
            y0 = rearrange_feature_map(x, direction='cw', channel_first=True)
            y = y0.view(B, 1, C, H * W).repeat(1, 4, 1, 1)
        ...
        if scans == 0:
            ...
            y0 = rearrange_feature_map(x, direction='ccw', channel_first=False)
            y0 = y0.view(B, H * W, 1, C).repeat(1, 1, 2, 1)
            y = rearrange_feature_map(x, direction='cw', channel_first=False)
            y = y.view(B, H * W, 1, C).repeat(1, 1, 2, 1)
            y = torch.cat([y0, y], dim=2)
        elif scans == 4:
            y0 = rearrange_feature_map(x, direction='ccw', channel_first=False)
            y = y0.view(B, H * W, 1, C).repeat(1, 1, 4, 1)
        elif scans == 5:
            y0 = rearrange_feature_map(x, direction='cw', channel_first=False)
            y = y0.view(B, H * W, 1, C).repeat(1, 1, 4, 1)
...
def cross_merge_fwd(y: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=0):
    from .scan4bevfeature import restore_feature_map
    if out_channel_first:
        ...
        if scans == 0:
            ...
            y0 = restore_feature_map(y[:, 0].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y1 = restore_feature_map(y[:, 1].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y2 = restore_feature_map(y[:, 2].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y3 = restore_feature_map(y[:, 3].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y = torch.cat([y0, y1, y2, y3], dim=1).sum(1)
        elif scans == 4:
            y0 = restore_feature_map(y[:, 0].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y1 = restore_feature_map(y[:, 1].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y2 = restore_feature_map(y[:, 2].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y3 = restore_feature_map(y[:, 3].squeeze(1), direction='ccw',channel_first=True).view(B, 1, D, -1)
            y = torch.cat([y0, y1, y2, y3], dim=1).sum(1)
        elif scans == 5:
            y0 = restore_feature_map(y[:, 0].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y1 = restore_feature_map(y[:, 1].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y2 = restore_feature_map(y[:, 2].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y3 = restore_feature_map(y[:, 3].squeeze(1), direction='cw',channel_first=True).view(B, 1, D, -1)
            y = torch.cat([y0, y1, y2, y3], dim=1).sum(1)
    else:
        ...
        if scans == 0:
            ...
            y0 = restore_feature_map(y[:, :, 0].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y1 = restore_feature_map(y[:, :, 1].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y2 = restore_feature_map(y[:, :, 2].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y3 = restore_feature_map(y[:, :, 3].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y = torch.cat([y0, y1, y2, y3], dim=2).sum(2)
        elif scans == 4:
            y0 = restore_feature_map(y[:, :, 0].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y1 = restore_feature_map(y[:, :, 1].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y2 = restore_feature_map(y[:, :, 2].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y3 = restore_feature_map(y[:, :, 3].squeeze(2), direction='ccw',channel_first=False).view(B, -1, 1, D)
            y = torch.cat([y0, y1, y2, y3], dim=2).sum(2)
        elif scans == 5:
            y0 = restore_feature_map(y[:, :, 0].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y1 = restore_feature_map(y[:, :, 1].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y2 = restore_feature_map(y[:, :, 2].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y3 = restore_feature_map(y[:, :, 3].squeeze(2), direction='cw',channel_first=False).view(B, -1, 1, D)
            y = torch.cat([y0, y1, y2, y3], dim=2).sum(2)
