...
class CustomOCCDepth4D(BEVDepth4D):
    def __init__(self, **kwargs):
        super(CustomOCCDepth4D, self).__init__(**kwargs)
    ...
    def extract_img_feat(self,
        ...
        Returns:
            x: [(B, C', H', W'), ]
            depth_cur_frame: (B*N_views, D, fH, fW)
        """
        if sequential:
        ...
        return [x], depth_cur_frame


@DETECTORS.register_module()
class CustomOCCDepth4D_FO(BEVDepth4D):
    def __init__(self, **kwargs):
        super(CustomOCCDepth4D_FO, self).__init__(**kwargs)
        self.extra_ref_frames = 1
        self.temporal_frame = self.num_frame
        self.num_frame += self.extra_ref_frames
        self.mask_net = nn.Sequential(  
            nn.Conv3d(64 * self.temporal_frame, 64, kernel_size=5, stride=1, padding=2, groups=2), 
            nn.BatchNorm3d(64),  
        ...
        self.conv3d = nn.Sequential(   
            nn.Conv3d(64 * self.temporal_frame, 64 * self.temporal_frame, kernel_size=3, stride=1, padding=1, groups=4),  # 分组卷积  
            nn.BatchNorm3d(64 * self.temporal_frame),  
            nn.ReLU(inplace=True),  
        )  
    ...
    def prepare_bev_feat(self, img, sensor2egos, ego2globals, intrin, post_rot, post_tran,
        ...
        # depth: (B * N, D, fH, fW)
        bev_feat, occ_feat, depth = self.img_view_transformer(
    ...
    def extract_img_feat(self,
        ...
        if self.align_after_view_transfromation:
            B, C, Dz, Dy, Dx = occ_feat_list[0].shape
            for adj_id in range(self.num_frame-2):
                bev_feat_list[adj_id] = self.shift_feature(
                    bev_feat_list[adj_id],  # (B, C, Dy, Dx)
                    [sensor2keyegos[0],     # (B, N_views, 4, 4)
                ...
                occ_feat_list[adj_id] = self.shift_feature(
                    occ_feat_list[adj_id].contiguous().view(B, C * Dz, Dy, Dx),  # (B, C * Dz, Dy, Dx)
                    [sensor2keyegos[0],     # (B, N_views, 4, 4)
                     sensor2keyegos[self.num_frame-2-adj_id]],  # (B, N_views, 4, 4)
                    bda  # (B, 3, 3)
                ).contiguous().view(B, C, Dz, Dy, Dx)    # (B, C * Dz, Dy, Dx)->(B, C, Dz, Dy, Dx)


        bev_feat = torch.cat(bev_feat_list, dim=1)
        occ_feat = torch.cat(occ_feat_list, dim=1)
        x = self.bev_encoder(bev_feat)
        x_occ, mask_fo = self.occ_fo_choose(occ_feat)
...
@DETECTORS.register_module()
class CustomOCCDepth4D_Stereo_FO(BEVDepth4D):
    def __init__(self, **kwargs):
        super(CustomOCCDepth4D_Stereo_FO, self).__init__(**kwargs)
        self.extra_ref_frames = 1
        self.temporal_frame = self.num_frame
        self.num_frame += self.extra_ref_frames
        self.mask_net = nn.Sequential(  
            ...
            nn.BatchNorm3d(64),  
            nn.ReLU(inplace=True), 
            nn.Conv3d(64, 1, kernel_size=1, stride=1, padding=0),  # 输出单通道mask  
            nn.Sigmoid()  # 将mask值限制在 [0, 1]  
        ) 
        self.conv3d = nn.Sequential(   
            nn.Conv3d(64 * self.temporal_frame, 64 * self.temporal_frame, kernel_size=3, stride=1, padding=1, groups=4),  # 分组卷积  
            nn.BatchNorm3d(64 * self.temporal_frame),  
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,
        ...
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]    # (B, C, Dy, Dx)
        return bev_feat, occ_feat, depth, stereo_feat
    ...
    def extract_img_feat(self,
        ...
        for fid in range(self.num_frame-1, -1, -1):
            ...
            if key_frame or self.with_prev:
                ...
                if key_frame:
                    bev_feat, occ_feat, depth, feat_curr_iv = \
                        self.prepare_bev_feat(*inputs_curr)
