...
class LSSViewTransformer(BaseModule):
    ...
    def __init__(
        ...
        self.depth_net = nn.Conv2d(
            in_channels, self.D + self.out_channels, kernel_size=3, padding=1)
    ...
    @force_fp32()
    def get_depth_loss_occlusion(self, depth_labels, depth_preds):
        """
        Args:
            depth_labels: (B, N_views, img_h, img_w) - Ground truth depth labels.
            depth_preds: (B*N_views, D, fH, fW) - Predicted depth probabilities.
        Returns: scalar loss.
        """
        # Downsample and encode the depth_labels to one-hot representation
        depth_labels = self.get_downsampled_gt_depth(depth_labels)  # (B*N_views*fH*fW, D)

        # Reshape depth_preds: (B*N_views, D, fH, fW) -> (B*N_views*fH*fW, D)
        depth_preds = depth_preds.permute(0, 2, 3, 1).contiguous().view(-1, self.D)

        # Compute foreground mask (only where there are non-zero labels)
        fg_mask = torch.max(depth_labels, dim=1).values > 0.0  # shape (B*N_views*fH*fW,)
        depth_labels = depth_labels[fg_mask]  # Foreground filtered labels: (num_fg, D)
        depth_preds = depth_preds[fg_mask]    # Foreground filtered preds: (num_fg, D)

        # Efficiently zero-out predictions after gt=1 index for all samples at once
        # Find the gt indices (locations where depth_labels == 1)
        gt_depth_indices = torch.argmax(depth_labels, dim=1)  # Shape: (num_fg,)

        # Create a range mask to zero-out future predictions
        batch_indices = torch.arange(depth_preds.size(0), device=depth_preds.device).unsqueeze(-1)  # Shape (num_fg, 1)
        range_mask = torch.arange(self.D, device=depth_preds.device).unsqueeze(0)  # Shape (1, D)
        zero_mask = range_mask > gt_depth_indices.unsqueeze(-1)  # Shape (num_fg, D)

        # Apply zero mask to depth_preds
        depth_preds = depth_preds * ~zero_mask  # Zero out values after gt=1 index

        # Compute binary cross entropy loss for the predictions and labels
        with autocast(enabled=False):
            depth_loss = F.binary_cross_entropy(
                depth_preds,
                depth_labels,
        ...
        # Scale the loss by depth loss weight and return
        return self.loss_depth_weight * depth_loss
...
@NECKS.register_module(force=True)
class Custom_LSSViewTransformer_FO(BaseModule):
    r"""Lift-Splat-Shoot view transformer with BEVPoolv2 implementation.
    ...
    def __init__(
        ...
        super(Custom_LSSViewTransformer_FO, self).__init__()
        self.loss_depth_weight = loss_depth_weight
        self.grid_config = grid_config
        self.downsample = downsample
        self.create_grid_infos(**grid_config)
        ...
        self.initial_flag = True
        self.collapse_z = collapse_z
        self.collapse_z_linear = nn.Sequential(
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ...
        bev_feat_shape = (depth.shape[0], int(self.grid_size[2]),
                          int(self.grid_size[1]), int(self.grid_size[0]),
                          feat.shape[-1])       # (B, Dz, Dy, Dx, C)
        occ_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths)    # (B, C, Dz, Dy, Dx)
        # collapse Z
        if self.collapse_z:
            bev_feat = torch.cat(occ_feat.unbind(dim=2), 1)     # (B, C*Dz, Dy, Dx)
            # 重塑 bev_feat 为 (B, Dy, Dx, C * Dz)（将通道移动到最后，以符合 nn.Linear 的输入）  
            bev_feat = bev_feat.permute(0, 2, 3, 1).contiguous()  
            # 应用 Linear 层，将 (B, Dy, Dx, C * Dz) 转换到 (B, Dy, Dx, C)  
            bev_feat = self.collapse_z_linear(bev_feat.view(-1, self.out_channels * 16))  # 展平为两维适应 Linear  
            bev_feat = bev_feat.view(occ_feat.shape[0], int(self.grid_size[1]), int(self.grid_size[0]), self.out_channels)  # 变回 (B, Dy, Dx, C)  
            # 将特征维度通道移回 (B, C, Dy, Dx)
            bev_feat = bev_feat.permute(0, 3, 1, 2).contiguous()  
        else:
            bev_feat = occ_feat
            
        return bev_feat, occ_feat
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            coor = self.get_ego_coor(*input[1:7])   # (B, N, D, fH, fW, 3)
            bev_feat, occ_feat = self.voxel_pooling_v2(
                coor, depth.view(B, N, self.D, H, W),
                tran_feat.view(B, N, self.out_channels, H, W))      # (B, C*Dz(=1), Dy, Dx)
        return bev_feat, occ_feat, depth
...
class LSSViewTransformer_FO_Depth(Custom_LSSViewTransformer_FO):
    def __init__(self, loss_depth_weight=3.0, depthnet_cfg=dict(), **kwargs):
        super(LSSViewTransformer_FO_Depth, self).__init__(**kwargs)
        self.loss_depth_weight = loss_depth_weight
    ...
    def forward(self, input, stereo_metas=None):
        ...
        bev_feat, occ_feat, depth= self.view_transform(input, depth, tran_feat)
        return bev_feat, occ_feat, depth
...
@NECKS.register_module()
class LSSViewTransformer_FO_Stereo(LSSViewTransformer_FO_Depth):
    def __init__(self,  **kwargs):
        super(LSSViewTransformer_FO_Stereo, self).__init__(**kwargs)
